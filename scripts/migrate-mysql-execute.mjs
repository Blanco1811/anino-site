  ...
  if (isRollback) {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        // 3. Delete Agents
        const delAgents = await tx.agent.deleteMany({
          where: { migrationRunId: migrationRun.id }
        });
        rollbackReport.deletedAgents = delAgents.count;

        // 4. Safely Delete Users (only if created in this run and have no new ANINO data)
        const usersInRun = await tx.user.findMany({
          where: { migrationRunId: migrationRun.id },
          include: {
            agents: { where: { NOT: { migrationRunId: migrationRun.id } } },
      ...
      console.log(`- Deleted Agent records: ${rollbackReport.deletedAgents}`);
